...
public class AreaCheck {
    int multiply(int a, int b) throws ArithmeticOverflowException {
        System.out.println(a);
        if (a>Integer.MAX_VALUE/4 || a<Integer.MIN_VALUE/4){
            throw new ArithmeticOverflowException();
        }
        if (b>Integer.MAX_VALUE/4 || b<Integer.MIN_VALUE/4){
            throw new ArithmeticOverflowException();
        }
    ...
    boolean isEqualArea(int w1, int h1, int w2, int h2){
        try {
            if (multiply(w1, h1) == multiply(w2, h2)){
                return true;
            }
