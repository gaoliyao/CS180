...
public class Project3StudentTestCases {
    ...
    public void testTeachersHaveIncreasingIDs() throws Exception {
        try {
            ...
            for (int i = 0; i <= expectedID; i++) {
                String name = NAMES[(i + 15) % NAMES.length];
                int age = 13;
            ...
            if (expectedID == lastTeacher.getID()) {
                fail("testTeachersHaveIncreasingIDs::Ensure that teacher IDs are increasing properly");
            }
        } catch (AssertionError e) {
            throw e;
        } catch (Exception e) {
    ...
    public void testProfessorsHaveIncreasingIDs() throws Exception {
        try {
            ...
            if (expectedID == lastProfessor.getID()) {
                fail("testProfessorsHaveIncreasingIDs::Ensure that professor IDs are increasing properly");
            }
        } catch (AssertionError e) {
            throw e;
        } catch (Exception e) {
...
//            e.printStackTrace();
    ...
    private static Teacher[] createTeachers(int numberOfTeachers) {
        Teacher[] teachers = new Professor[numberOfTeachers];
        for (int i = 0; i < numberOfTeachers; i++) {
            String name = NAMES[Math.abs((numberOfTeachers * 5) % NAMES.length)];
            int age = 40 + Math.abs((numberOfTeachers * 5) % 80);
            String gender = "Unspecified";
            teachers[i] = new Professor(name, age, gender, Math.abs((1000 * i) % 10000), Math.abs((100 * i) % 1000));
        }
        return teachers;
    }
